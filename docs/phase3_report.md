...
### モデル別の可視化
各モデルごとに以下のプロットが作成されました（`results/plots/{model}/`）：
...
### モデル間比較の可視化
`results/plots/cross_model/` に以下のプロットが作成されました：
...
### 感情方向ベクトル
- `results/emotion_vectors/gpt2_vectors.pkl`
- `results/emotion_vectors/pythia-160m_vectors.pkl`
- `results/emotion_vectors/gpt-neo-125m_vectors.pkl`
...
### モデル間類似度テーブル
- `results/cross_model_similarity.csv`: モデル間の類似度を表形式で保存
