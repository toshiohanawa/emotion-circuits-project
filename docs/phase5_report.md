...
- **感情方向ベクトル**:
  - 文末ベース: `results/emotion_vectors/gpt2_vectors.pkl`
  - 感情語トークンベース: `results/emotion_vectors/gpt2_vectors_token_based.pkl`
...
python src/models/activation_patching_sweep.py \
  ...
  --output results/patching/gpt2_sweep_token_based.pkl \
  --layers 3 5 7 9 11 \
...
python src/models/activation_patching_sweep.py \
  --model gpt2 \
  --vectors_file results/emotion_vectors/gpt2_vectors.pkl \
  --prompts_file data/neutral_prompts.json \
  --output results/patching/gpt2_sweep_sentence_end.pkl \
  --layers 3 5 7 9 11 \
  --alpha -2 -1 -0.5 0 0.5 1 2
```

**生成されたヒートマップ**:
- `results/plots/heatmaps_token_based/`: 感情語トークンベースのヒートマップ（15ファイル）
- `results/plots/heatmaps_sentence_end/`: 文末ベースのヒートマップ（15ファイル）
...
python src/analysis/compare_patching_methods.py \
  --model gpt2 \
  --sentence_end_vectors results/emotion_vectors/gpt2_vectors.pkl \
  --token_based_vectors results/emotion_vectors/gpt2_vectors_token_based.pkl \
  --prompts_file data/neutral_prompts.json \
  --output_dir results/patching/comparison \
  --layers 3 5 7 9 11 \
  --alpha -2 -1 -0.5 0 0.5 1 2
```

**生成された比較プロット**:
- `results/patching/comparison/comparison_*.png`: 各感情×各メトリクスの比較プロット（15ファイル）
...
1. **スイープ結果ファイル**:
   - `results/patching/gpt2_sweep_token_based.pkl`: 感情語トークンベースの全結果
   - `results/patching/gpt2_sweep_sentence_end.pkl`: 文末ベースの全結果
...
1. **比較結果ファイル**:
   - `results/patching/comparison/gpt2_comparison.pkl`: 両方法の比較結果
...
### データファイル
- `results/patching/gpt2_sweep_token_based.pkl`: 感情語トークンベースのスイープ結果
- `results/patching/gpt2_sweep_sentence_end.pkl`: 文末ベースのスイープ結果
- `results/patching/comparison/gpt2_comparison.pkl`: 比較実験の結果

### 可視化ファイル
- `results/plots/heatmaps_token_based/*.png`: 感情語トークンベースのヒートマップ（15ファイル）
- `results/plots/heatmaps_sentence_end/*.png`: 文末ベースのヒートマップ（15ファイル）
- `results/patching/comparison/comparison_*.png`: 比較プロット（15ファイル）
