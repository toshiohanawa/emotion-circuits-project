...
## ドキュメント参照順序
- README の「ドキュメントマップ」で示した依存関係に従い、仕様変更時は **README → docs/proposal/masterplan.md → docs/implementation_plan.md → 本ノート → docs/report/** の順で反映する。
- 本ノートは実行ログとランブックを提供する「下流文書」に位置づけられるため、前提が変わったら `git blame` で該当セクションを更新し、更新根拠として上流ドキュメントの該当節を明記する。
- `docs/archive/` 配下は歴史的参照のみであり、本ノートの更新対象に含めない。
