...
def load_model_vectors(model_names: List[str], vectors_dir: Path) -> Dict[str, Dict[str, np.ndarray]]:
    """
    複数モデルの感情ベクトルを読み込む
    ...
    for model_name in model_names:
        vectors_file = vectors_dir / f"{model_name}_vectors.pkl"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Cross-model emotion vector analysis")
    ...
    parser.add_argument("--output_dir", type=str, default="results/plots/cross_model", help="Output directory")
    parser.add_argument("--output_table", type=str, default="results/cross_model_similarity.csv", help="Output CSV file")
    
    args = parser.parse_args()
    
    # モデルベクトルを読み込み
    print("Loading model vectors...")
    model_vectors = load_model_vectors(args.models, Path(args.vectors_dir))
    ...
    for emotion_label in emotion_labels:
        visualizer.plot_cross_model_similarity(
            model_vectors,
            emotion_label,
            save_path=visualizer.output_dir / f"cross_model_{emotion_label}.png"
        )
