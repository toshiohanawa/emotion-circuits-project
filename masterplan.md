...
概念フレームワーク（3.1）と技術的な3本柱（3.2）は独立したものではなく、それぞれ次のように対応している。

| 概念フレームワーク            | 主に関わる技術的柱 | 代表的なフェーズ例                                   |
| ----------------------------- | ------------------ | ---------------------------------------------------- |
| 書き込み（write-in）          | （補助的に T1/T2） | Phase 6（Head screening）                            |
| 伝搬（propagation）           | （主に観察レベル） | Phase 3〜5（層ごとのベクトル解析、サブスペース解析） |
| 読み出し（read-out）          | T1, T2             | Phase 7（Head patching）                             |
| 出力影響（behavioral effect） | T1, T2, T3         | Phase 5（層×αスイープ）、Phase 7.5（統計的厳密性） |

この対応を意識しておくことで、「今やっている作業は感情の流れのどの部分を見ているのか」「どの技術的柱を強化しているのか」が把握しやすくなる。
...
#### ✅ Phase 8：中規模モデルへのスケーリング（感情回路の"成長"を見る）

 **目的** : 160Mクラスで見つけた構造が、8B〜12Bといった中規模モデルでどう変化・安定化するかを調べる。

 **やったこと** :

* Llama3 8B / Gemma3 12B / Qwen3 8B を対象に、GPT-2の感情サブスペースとのアライメント実験を実施
* token-based 感情ベクトル → 多サンプルPCA → neutral から線形写像学習 → before/after overlap の比較
* モデル固有の汎化パターンを発見：
  * **Llama3 8B**: 優秀なアライメント（Layer 11 で Δoverlap = 0.713）
  * **Qwen3 8B**: 中程度のアライメント（Layer 4 で Δoverlap = 0.105）
  * **Gemma3 12B**: 低調（数値的不安定性、Δoverlap < 0.001）
* `src/models/phase8_large/registry.py` でモデル定義を一元管理
* `src/analysis/run_phase8_pipeline.py` で自動化パイプラインを実装

 **このフェーズの意味** : 「小さな脳で見つかった感情回路が、大きな脳ではどう変化するか」という問いに対して、**アーキテクチャ依存性が決定的**という重要な知見を得た。線形写像によるアライメントが有効なモデル（Llama3）と、より複雑な構造を必要とするモデル（Gemma3）が存在することが明らかになった。
...
| パッケージ | 対応フェーズ | 主題                                                                   | 状態 |
| ---------- | ------------ | ---------------------------------------------------------------------- | ---- |
| Paper 1    | Phase 1〜8 | 軽量〜中規模モデルにおける感情サブスペース、残差パッチング、headレベル因果回路、統計的厳密性、スケーリング検証 | ✅ 完了 |
| Paper 2    | Phase 9   | 感情回路の構造的解剖（OV/QKとmulti-head回路）                   | 🔭 これから |
| Paper 3    | Phase 10〜11 | 多言語・多属性への一般化と、感情回路を用いた制御・応用                 | 🔭 これから |

この区切りを意識しておくことで、「今どのパッケージのどの部分を進めているのか」「卒論・修論としてどこまでをまとめるか」が見通しやすくなる。

**2025-11-16 更新**: Phase 8 が完了したことで、Paper 1 のスコープが拡大し、小型〜中規模モデルまでの包括的な研究として完成度が高まった。
...
* Llama3 8B / Gemma3 12B / Qwen3 8B を対象に、GPT-2の感情サブスペースとのアライメント実験を実施
* モデル固有の汎化パターンを発見（Llama3: 優秀、Qwen3: 中程度、Gemma3: 低調）
* アーキテクチャ依存性が決定的であることを実証

 **このフェーズの意味** : 線形写像によるアライメントが有効なモデルと、より複雑な構造を必要とするモデルが存在することが明らかになった。
...
研究を進めていく中で、「次に何をすべきか」「この方向で良いのか」と迷うタイミングは必ず来る。そのときに立ち返るための簡易チェックリストをここに置いておく。

#### 7.1 今、どのフェーズにいるか？

* [ ] Phase 0〜8 のどこかを振り返っている（過去の航海ログを読みたいとき）

  → Section 4 の該当フェーズを読み返す。
* [ ] Phase 8 の結果を深掘りしている（中規模モデルの汎化性を分析したいとき）

  → Phase 8 のアーキテクチャ依存性に注目し、Llama3/Qwen3/Gemma3 の違いを検討する。
* [ ] Phase 9 以降（構造的解剖／多言語／応用）を計画している（未来の航路を描きたいとき）

  → Section 4 の「これから」のフェーズ記述を読み返す。

#### 7.2 迷ったときの3つの問い

1. **この作業は Q1/Q2/Q3 のどれに答えようとしているか？**
   * Q1: 単一モデル内部の「感情の流れ」か？
   * Q2: モデル間の共通サブスペースか？
   * Q3: 因果的な操作可能性か？

     → どれにも当てはまらないと感じたら、そもそもの目的を一度言語化し直す。
2. **これは、観察・因果・構造・アライメント・回路のどの段階か？**
   * 観察（Observation）
   * 因果（Causal Intervention）
   * 構造（Structure / Subspace）
   * アライメント（Alignment / Linear Map）
   * 回路（Circuit / Multi-head + MLP）

     → どの段階かが曖昧なときは、いきなり回路に飛びついていないかを確認する。
3. **この作業は、T1/T2/T3 のどの柱を強化しているか？**
   * T1: Multi-token Patching
   * T2: Transformer-based Evaluation
   * T3: Random Baseline & Statistical Testing

     → どの柱にも関わらない場合、その作業の技術的な意義を再確認する。

#### 7.3 行き詰まったときの処方箋

* **実験結果が期待と違ったとき**

  → Section 3.3 の「失敗を起点に問いを深める」に戻り、データ・見方・変換のどれを変えるかを検討する。
* **やることが多すぎて圧倒されたとき**

  → 論文パッケージ（Paper 1〜3）のどれに集中したいのかを決め、そのパッケージに関係するフェーズだけを一時的にフォーカスする。
* **モチベーションが下がったとき**

  → Section 6.1「この研究の好きなところ」を読み返し、「なぜこの旅を始めたのか」を思い出す。
